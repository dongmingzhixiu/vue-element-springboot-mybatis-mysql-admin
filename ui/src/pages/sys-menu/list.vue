<template>
	<div class="box-b p10 over-h">
		<el-row v-loading="loading">
			<div class="f-b w">
				<div style="flex-shrink: 2;width: calc(100% - 600px);">
					...
					<el-card class="box-b m-t10" shadow="hover">
						<page-helper :change-list="menuChangeList" ref="pageHelper" :add-height="-220" :api-name="apiName" :api-data="apiData"
						 :is-init-data="true" :columns="columns" :tools="tools">
				...
				<div class="flex-no-chang-width" style="flex-shrink: 0;width: 600px;height: calc(100vh - 121px);margin-top: -21px;">
						...
						<div v-if="selectRow&&Object.keys(selectRow).length>0" class="w box-b p-t2" style="height: 100%;">
							...
							<el-tabs v-if="buttons&&buttons.filter(btn=>btn.menuType==7||btn.menuType==8).length>0" type="border-card"
							 v-model="rightActive" class="w box-b" style="padding: 0; height: calc(100% - 44px);">
								<el-tab-pane v-if="buttons&&buttons.filter(btn=>btn.menuType==8).length>0" label="按钮设置" name="tab-first" class="box-b"
								 style="padding: 0;height: calc(100% - 44px);">
									<el-tabs v-model="active" style="padding: 0;margin-top: -10px;padding-left: 10px;box-sizing: border-box;height: calc(100vh - 220px);">
										<el-tab-pane label="已设置按钮" name="first" style="padding: 0;overflow-y: auto;height: calc(100vh - 280px);">
											<div class="f-s" style="flex-wrap: wrap;">
												<template v-for="(item,i) in  buttonList">
													<el-button class="m4 f-c" :type="item.type" :style="item.css" :size="item.size" :key="i" :icon="item.icon">
										...
										<el-tab-pane :label="buttonQueryId?'编辑按钮':'新增按钮'" name="second" style="padding: 0;overflow-y: auto;height: calc(100vh - 280px);">
											<page-editor style="margin-left: -10px;" :change-form="buttonChangeForm" :btn="{'save':'保存按钮',close:'取消'}"
											 :layout="buttonLayout" :method="buttonMethod" :add="buttonAdd" :query-id="buttonQueryId" :readonly='buttonReadonly'
											 @save="buttonSubmit" @save-before="buttonSaveBefore" @close-tab="buttonCancel"></page-editor>
										</el-tab-pane>
									</el-tabs>
								</el-tab-pane>
								<el-tab-pane v-if="buttons&&buttons.filter(btn=>btn.menuType==7).length>0" label="布局设置" name="tab-second" class="box-b"
								 style="padding: 0;height: calc(100% - 44px);">
									<el-tabs v-model="active" style="padding: 0;margin-top: -10px;padding-left: 10px;box-sizing: border-box;height: calc(100vh - 220px);">
											...
											<template v-if="item.layoutType==tab.type" v-for="(item,i) in  layoutList">
												...
												<div :key="i" v-else>
													...
													<template v-if="tab.template=='page-editor'">
														...
														<page-editor :readonly="false" :add="true" :key="i" style="width: 100%;margin-left: -40px;" :btn="null"
														 :layout="getLayoutJson(item.layoutJson)">
															<template slot="tools-btn">
																<el-button plain type="primary" @click="btnClick(item,'layoutEditor')">修改</el-button>
															</template>
														</page-editor>
													</template>
													<template v-if="tab.template=='page-echarts'">
														<!-- <div v-if="item.remake" class="f-c tip-w b-w1">{{item.remake}}</div>
														<page-editor :readonly="false" :add="true" :key="i" style="width: 100%;margin-left: -40px;" :btn="null"
														 :layout="getLayoutJson(item.layoutJson)">
															<template slot="tools-btn"> -->
														<div class="w f-c align-items-center" style="height: 400px;">图表布局不可直接预览，请点击修改</div>
														<el-button plain type="primary" @click="btnClick(item,'layoutEditor')">修改</el-button>
														<!-- 	</template>
														</page-editor> -->
													</template>
										...
										<el-tab-pane v-if="documentLayout&&documentLayout.length>0" :label="documentAdd?'添加布局':'修改布局'" name="settingLayout"
										 style="overflow-y: auto;height: calc(100vh - 280px);">
											<page-editor :form="{layoutJson:codeEditorJson}" style="width: 100%;margin-left: -40px;" :change-form="documentChangeForm"
											 :btn="{'save':'保存布局',close:'取消'}" :layout="documentLayout" :method="documentMethod" :add="documentAdd"
											 :query-id="documentQueryId" :readonly='documentReadonly' @save="documentSubmit" @save-before="documentSaveBefore"
											 @close-tab="documentCancel">
												<template slot="textarea-append" slot-scope="row">
													<el-button v-if="row.row.prop=='layoutJson'" @click="openCodeEidtor(row.form)" type="danger" size="mini"
													 plain>调试布局</el-button>
													<layout-code-editor v-if="showCodeEditor&&row.form.layoutType!=4" :show-ok-button="true" :layout-code='codeEditorJson'
													...
													<echarts-code-editor v-if="showCodeEditor&&row.form.layoutType==4" :show-ok-button="true" :layout-code='codeEditorJson'
													 class="w" :type="1" style="position: fixed;left :0;right: 0;top: 0;z-index: 9999;" @code="getCodeEditor"
													 @close="showCodeEditor=!showCodeEditor"></echarts-code-editor>
												</template>
											</page-editor>
										</el-tab-pane>
