<template>
	<div class="PageHelper box-b" :style="{'height':height?height:`calc(100vh + ${addHeight}px)`}">
		...
		<el-table @selection-change="changeCheckBoxValue" @select="rowSelect" @select-all="selectAll" :class="{'checkbox':(tableType=='checkbox'&&elTableType=='')||(tableType==''&&elTableType=='expand'),'checkbox-expand':tableType=='checkbox'&&elTableType=='expand'}"
		 v-if="columns&&columns.length>0" row-key="id" ref="elTable" empty-text="没有找到相关数据" fit border :height="height?`calc(height - 65px)`:`calc(100vh + ${addHeight-65}px)`"
		 :style="{'width':'100%','height':height?`calc(height - 65px)`:`calc(100vh + ${addHeight-65}px)`,'box-sizing': 'border-box'}"
		 v-loading="loading" :data="listData" :row-style="rowClass" @row-click="rowClick" style="width: 100%">
			...
			<el-table-column v-if="(col.visabled==undefined||col.visabled==true)&&columns&&columns.length>0&&col&&col.prop&&col.label"
			 v-for="(col,index) in columns" :prop="col.prop" :label="col.label" :width="col.width?col.width:defaults.width" :key="index">
				<template slot-scope="scope">
		...
		<el-pagination v-if="columns&&columns.length>0&&showPageHelper" align="right" background @size-change="handleSizeChange"
		...
		<div v-else-if="showPageHelper&&(!columns||columns.length<=0)" class="f-c h-240 color8">没有访问权限或暂未找到相关的配置信息！</div>
	</div>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			getSelectAfterList() {
				try {
					let columns = this.columns.filter(r => r.select);
					let temp = {};
					columns.map(r => {
						let key = r['prop'];
			...
			getPageData() {
				...
				try {
					...
					this.$api[this.apiName](data).then(res => {
						...
						res = res.data;
						this.showPageHelper = !(res instanceof Array);
